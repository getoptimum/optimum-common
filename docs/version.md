...
Include this package in any service that needs to expose build information.
Calling `version.GetVersion()` and `version.GetCommitHash()` allows other
repositories to report consistent metadata in logs, health checks or CLI
commands without custom `-ldflags` wiring.

## Examples

A runnable program in [examples/versioning](../examples/versioning) shows how to
print the current module version, commit hash and how `DeriveVersion` normalises
various pseudo-version strings.

## Currently
The version and last hash commit are injected via ldflags at build