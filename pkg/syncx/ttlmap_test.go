...
func TestTTLMap(t *testing.T) {
	...
	t.Run("DoAndApply method should modify value", func(t *testing.T) {
		// given
		m := syncx.NewTTLMap[string, int](time.Minute, time.Minute)
		m.Put("a", 1)
		val, ok := m.Get("a")
		require.True(t, ok)
...
func TestClose_IsIdempotent_AndStopsJanitor(t *testing.T) {
	m := syncx.NewTTLMap[string, int](time.Millisecond*10, time.Millisecond*5)
...
func TestWithContext_CancelStopsJanitor(t *testing.T) {
	...
	m := syncx.NewTTLMapWithContext[string, int](ctx, 100*time.Millisecond, 10*time.Millisecond)
