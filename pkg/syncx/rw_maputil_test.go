...
import (
	...
	test_syncx "github.com/getoptimum/optimum-common/pkg/test_utils"
...
func TestRWMapBasicOperations(t *testing.T) {
	...
	for name, ctor := range map[string]func() *syncx.RWMap[int, string]{
		"empty":    func() *syncx.RWMap[int, string] { return syncx.NewRWMap[int, string]() },
		"from map": func() *syncx.RWMap[int, string] { return syncx.NewRWMapFromStdMap(make(map[int]string)) },
	} {
	...
	for i := 0; i < numGoroutines; i++ {
		for _, f := range funcsList {
			...
			go func(i int, f func(i, j int)) {
				defer wg.Done()
				test_syncx.RunConcurrently(numFunctions, numOperations, func() { f(i, i) })
			}(i, f)
		}
	}
	wg.Wait()
...
func Test_RWMap_LoadAllAndErase(t *testing.T) {
	m := syncx.NewRWMap[string, string]()
	m.Store("a", "A")
	m.Store("b", "B")
