...
func TestCleanupFolders_CreateAndClean(t *testing.T) {
	...
	require.NoError(t, optio.CleanupFolders([]string{a, b}, "keep.txt"))
...
func TestCleanupFolders_IgnoreDoesNotRemoveDirectoriesWithSameNameAsIgnoredFile(t *testing.T) {
	...
	require.NoError(t, optio.CleanupFolders([]string{dir}, "keepme"))
...
func TestAtomicallySaveToFile_PrefixChecksumFormat(t *testing.T) {
	...
	require.NoError(t, optio.AtomicallySaveToFile(target, payload))

	// read raw file and verify first optio.ChecksumSize bytes == crc64(remaining)
	// #nosec G304 -- target is generated in a safe test TempDir; not user-controlled
	raw, err := os.ReadFile(target)
	require.NoError(t, err)
	require.GreaterOrEqual(t, len(raw), optio.ChecksumSize)

	wantCRC := crc64.Checksum(payload, crc64.MakeTable(crc64.ISO))
	gotCRC := binary.LittleEndian.Uint64(raw[:optio.ChecksumSize])
	require.Equal(t, wantCRC, gotCRC)
	require.True(t, bytes.Equal(payload, raw[optio.ChecksumSize:]))
}

func TestAtomicallySaveToFile_EmptyPayload(t *testing.T) {
	tmp := t.TempDir()
	target := filepath.Join(tmp, "empty.bin")
	require.NoError(t, optio.AtomicallySaveToFile(target, nil))

	// should still be optio.ChecksumSize bytes (crc of empty slice)
	// #nosec G304 -- target is generated in a safe test TempDir; not user-controlled
	raw, err := os.ReadFile(target)
	require.NoError(t, err)
	require.Equal(t, optio.ChecksumSize, len(raw))

	data, err := optio.LoadFromFile(target)
	require.NoError(t, err)
...
func TestAtomicallySaveToFile_PreserveExistingMode(t *testing.T) {
	...
	require.NoError(t, optio.AtomicallySaveToFile(target, []byte("content")))
...
func TestLoadFromFile_PathIsCleaned(t *testing.T) {
	...
	data, err := optio.LoadFromFile(cleaned)
	require.NoError(t, err)
