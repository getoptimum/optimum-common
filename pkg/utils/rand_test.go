...
func TestMsgHashWithTimestamp_Determinism(t *testing.T) {
	...
	h1 := randutil.MsgHashWithTimestamp(topic, msg, timestamp)
	h2 := randutil.MsgHashWithTimestamp(topic, msg, timestamp)
	require.Equal(t, h1, h2, "same inputs should yield same hash")

	// Different timestamp should produce different hash
	h3 := randutil.MsgHashWithTimestamp(topic, msg, timestamp+1)
	...
	h4 := randutil.MsgHashWithTimestamp(topic, msg, timestamp-1)
	require.NotEqual(t, h1, h4)
}
...
func TestMsgHashWithTimestamp_DifferentFromMsgHash(t *testing.T) {
	...
	hashWithTS := randutil.MsgHashWithTimestamp(topic, msg, timestamp)
	hashWithoutTS := randutil.MsgHash(topic, msg)
...
func TestMsgHashWithTimestamp_IsHex_And_Length(t *testing.T) {
	...
	h := randutil.MsgHashWithTimestamp("t", []byte("m"), 1234567890)
	require.NotEmpty(t, h)
